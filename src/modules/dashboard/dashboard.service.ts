  ...
  async getStats(clientId: string) {
    const [
      ...
      this.prisma.order.aggregate({
        where: { clientId, status: 'PAID', createdAt: { gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } },
        _sum: { total: true },
      }).then(res => res._sum.total || 0),
      this.prisma.appointment.count({ where: { clientId, date: { gte: new Date() } } })
    ]);
